...
public class FormatFinanceCondition {
  ...
  public static CreditDeliveryFinanceCondition forCreditDelivery(FinanceCondition fc){
    ...
    return new CreditDeliveryFinanceCondition(fc.getId().toString(),fc.getName(),fc.getInterestRate(),fc.toAmount(),dlSubsidy,fd,fc.getUtm());
  }


  public static FinanceConditionData from(FinanceCondition fc){
    var dlSubsidy = new SubsidyData(fc.getDealerSubsidy().getId(),fc.getDealerSubsidy().getName(),fc.getDealerSubsidy().getRate(),fc.getDealerSubsidy().getType().name(),fc.getDealerSubsidy().getMaxTimeLoan().toString());
    var fcSubsidy = new SubsidyData(fc.getFactorySubsidy().getId(),fc.getFactorySubsidy().getName(),fc.getFactorySubsidy().getRate(),fc.getFactorySubsidy().getType().name(),fc.getFactorySubsidy().getMaxTimeLoan().toString());
    var fd = new FlatData(fc.getFlat().getId(),fc.getFlat().getName(),fc.getFlat().getRate());
    return new FinanceConditionData(fc.getId().toString(),fc.getName(),fc.getInterestRate(),fc.toAmount(),dlSubsidy,fcSubsidy,fd,fc.getUtm());
  }
